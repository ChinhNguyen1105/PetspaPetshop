...
import { PetService } from 'src/modules/pets/service/pet.service';
...
export class PetServiceImpl implements PetService {
  ...
  async createPet(
    ...
    this.logger.log(
      '[PET] Thêm thú cưng mới cho user hiện tại',
    );

    const currentUser =
      await this.userService.getUserLogin();

    await this.checkExistPet(
      currentUser.id,
      req.name,
    );
  ...
  async updatePet(
    ...
    this.logger.log(
      `[PET] Cập nhật thú cưng ID: ${req.id}`,
    );
    ...
    this.logger.log(
      `[PET] Cập nhật thành công thú cưng ID: ${req.id}`,
    );
  ...
  async deletePet(
    ...
    this.logger.log(
      `[PET] Xóa thành công thú cưng ID: ${id}`,
    );
  ...
  async deactivatePet(
    ...
    this.logger.log(
      `[PET] Khóa thành công thú cưng ID: ${id}`,
    );
  ...
  async activatePet(
    ...
    this.logger.log(
      `[PET] Mở khóa thành công thú cưng ID: ${id}`,
    );
  ...
  async getMyPets(): Promise<PetDto[]> {
    this.logger.log(
      '[PET] Lấy danh sách thú cưng của user hiện tại',
    );
    ...
    const pets =
      await this.petRepository
        .findByUserIdAndDeleteFlagFalse(
          currentUser.id,
        );
  ...
  async getPetDetail(
    ...
    this.logger.log(
      `[PET] Lấy chi tiết thú cưng ID: ${id}`,
    );
    ...
    this.logger.log(
      `[PET] Lấy chi tiết thành công thú cưng ID: ${id}`,
    );
  ...
  private async getPetById(
    petId: number,
  ): Promise<Pet> {
    const pet =
      await this.petRepository
        .findByIdAndDeleteFlagFalse(
          petId,
        );

    if (!pet) {
      this.logger.warn(
        `[NOT_FOUND] Không tìm thấy thú cưng ID: ${petId}`,
      );
