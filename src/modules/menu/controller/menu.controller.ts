...
export class MenuController {
  constructor(
    @Inject(PROVIDER_TOKEN.MENU_SERVICE)
    private readonly menuService: MenuService,
  ) {}

  @Post(UrlConstant.Menu.CREATE_MENU)
  async createMenu(@Body() req: ReqForMenuDto) {
  ...
  @Put(UrlConstant.Menu.UPDATE_MENU.replace('{id}', ':id'))
  async updateMenu(@Param('id') id: number, @Body() req: ReqForMenuDto) {
    return VsResponseUtil.successWithStatus(
      HttpStatus.OK,
  ...
  @Delete(UrlConstant.Menu.DELETE_MENU.replace('{id}', ':id'))
  ...
  async getAllMenus(@Query('page') page = 1, @Query('pageSize') pageSize = 10) {
    return VsResponseUtil.successWithStatus(
      HttpStatus.OK,
