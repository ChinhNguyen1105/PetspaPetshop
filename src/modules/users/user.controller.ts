...
export class UserController {
  ...
  @Get(UrlConstant.User.GET_USER) async getUser(@Param('id') id: string) {
    const userDto: UserDto = await this.userService.getUserById(id);
    return VsResponseUtil.successWithStatus(HttpStatus.OK, userDto);
  }
  @Post(UrlConstant.User.CREATE_USER) async createUser(
    @Body() userCreateDto: UserCreateDto,
  ) {
    const userDto: UserDto = await this.userService.createUser(userCreateDto);
    return VsResponseUtil.successWithStatus(HttpStatus.OK, userDto);
  }
  @Put(UrlConstant.User.UPDATE_USER) async updateUser(
    @Body() user: UserUpdateDto,
  ) {
    const userDto: UserDto = await this.userService.updateUser(user);
    return VsResponseUtil.successWithStatus(HttpStatus.OK, userDto);
  }
  @Delete(UrlConstant.User.DELETE_USER) async deleteUser(
    @Param('id') id: string,
  ) {
    const dto: CommonResponseDto = await this.userService.deleteUser(id);
    return VsResponseUtil.successWithStatus(HttpStatus.OK, dto);
  }
  @Get(UrlConstant.User.GET_USERS) async getAllUser(
    ...
    return VsResponseUtil.successWithStatus(
      HttpStatus.OK,
      await this.userService.getAllUser(filter, Number(page), Number(pageSize)),
    );
  }
  @Patch(UrlConstant.User.CHANGE_USER_STATUS) async changeUserStatus(
    @Param('id') id: string,
  ) {
    const dto: CommonResponseDto = await this.userService.changeUserStatus(id);
    return VsResponseUtil.successWithStatus(HttpStatus.OK, dto);
  }
  ...
  async addAvatar(
    ...
    const commonResponseDto: CommonResponseDto =
      await this.userService.addAvatar(userId, file);
    return VsResponseUtil.successWithStatus(HttpStatus.OK, commonResponseDto);
  }
  @Put(UrlConstant.User.UPDATE_PROFILE) async updateProfile(
    @Body() reqUserUpdateProfile: ReqUserUpdateProfileDto,
  ) {
